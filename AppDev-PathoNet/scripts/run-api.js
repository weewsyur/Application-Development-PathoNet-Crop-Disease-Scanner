...
function findPythonExecutable() {
  ...
  // Check if venv exists but python executable is missing
  const venvDir = path.join(projectRoot, '.venv');
  if (fs.existsSync(venvDir)) {
    console.warn('⚠️  Virtual environment exists but Python executable not found');
    console.log('   Run setup script to fix: npm run setup:python');
  }

  // Fall back to system python
  console.log('ℹ No venv found, using system Python');
  console.log('   Run setup script to create venv: npm run setup:python');
...
function startAPI() {
  ...
  const scriptPath = path.join(projectRoot, 'run_api_server.py');
