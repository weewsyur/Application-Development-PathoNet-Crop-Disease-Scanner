...
def check_pathonet_module():
    """Check that app/components/PathoNetV1.py exists and imports."""
    root = os.path.dirname(os.path.abspath(__file__))
    path_py = os.path.join(root, "app", "components", "PathoNetV1.py")
    if not os.path.isfile(path_py):
        print(f"❌ PathoNetV1.py not found at:\n   {path_py}")
        return False
    print(f"✓ PathoNetV1.py present:\n   {os.path.abspath(path_py)}")

    sys.path.insert(0, os.path.join(root, "app", "components"))
    try:
        import importlib.util

        spec = importlib.util.spec_from_file_location("PathoNetV1", path_py)
        if spec is None or spec.loader is None:
            print("❌ Could not load spec for PathoNetV1.py")
            return False
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        if not hasattr(mod, "run_flask_server_v2"):
            print("❌ PathoNetV1.py missing run_flask_server_v2")
            return False
        print(f"✓ PathoNetV1 loaded from: {os.path.abspath(mod.__file__)}")
        return True
